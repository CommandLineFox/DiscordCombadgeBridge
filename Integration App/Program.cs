        ...
        static async Task RunDiscordLoopAsync(CancellationToken cancellationToken)
        {
            while (!cancellationToken.IsCancellationRequested)
            {
                try
                {
                    ...
                    long appliedVersion = snapshot.Version;

                    Console.WriteLine(snapshot.Muted
                        ? "Initial Discord state applied: muted."
                        : "Initial Discord state applied: unmuted.");

                    while (!cancellationToken.IsCancellationRequested)
                    {
                        await Task.Run(() => WaitHandle.WaitAny([stateChangedEvent, cancellationToken.WaitHandle]), cancellationToken);
                        ...
                        if (appliedVersion != snapshot.Version)
                        {
                            ...
                            Console.WriteLine(snapshot.Muted
                                ? "Discord state changed via OSC: muted."
                                : "Discord state changed via OSC: unmuted.");
                        }
                    }
                }
